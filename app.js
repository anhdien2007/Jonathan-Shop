...
const SHEET_WEBAPP_URL = "https://script.google.com/macros/s/AKfycbxhY-a-9KQVQMUxXYWSvqq7u9bal1DNbpt64E2OUbe9L-Chr4dEaNoBweSRzIApom0N/exec";
const PRODUCTS = [
  {
    id: "cue-1",
  ...
  },
  {
    ...
    rating: 4.9,
    stock: 1,
    tags: ["Combo", "Value", "Affordable"],
    desc: "Complete set with multiple cues for all your needs.",
    image: "images/cue3.jpg"
  },
  {
    id: "cue4",
    name: "Colorful Cue Stick",
    category: "cue",
    price: 550.0,
    rating: 4.7,
    stock: 5,
    tags: ["Colorful", "Unique", "Stylish"],
    desc: "A vibrant and unique cue stick for those who want to stand out.",
    image: "images/cue4.jpg"
  },
  {
    id: "cusecase2",
    ...
    rating: 4.9,
    stock: 4,
    tags: ["Luxury", "Leather", "Protective"],
    desc: "Premium leather cue case offering superior protection and style.",
    image: "images/cuecase2.jpg"
  }
];
...
async function sendOrderToSheet(order) {
  const res = await fetch("https://formspree.io/f/mojvbwkr", {
    method: "POST",
    headers: {
  ...
  // Formspree success is HTTP 200 OK
  if (!res.ok) {
    throw new Error(data.error || data.message || `Formspree error (HTTP ${res.status})`);
  }
...
const paynowBackdrop = document.getElementById("paynowBackdrop");
const closePaynow = document.getElementById("closePaynow");
...
if (closePaynow) closePaynow.addEventListener("click", () => { if (paynowBackdrop) paynowBackdrop.style.display = "none"; });
...
placeOrderBtn.addEventListener("click", async () => {
  ...
  if (!name || !phone || !address) {
    return alert("Please fill in Name, Phone, Address.");
  }

  // Total shown in UI already:
  const totalText = totalEl.textContent.replace("$","").replace("FREE","0");
  ...
  try {
    ...
    const result = await sendOrderToSheet(order);

    if (!result.ok) throw new Error(result.error || "Failed");
    okMsg.style.display = "block";
    okMsg.textContent = `✅ Order sent! Order ID: ${order.orderId}`;

    // If PayNow chosen, open PayNow QR screen (optional)
    if (method === "PayNow") {
      paynowBackdrop.style.display = "flex";
    }
    ...
    alert("❌ Failed to send order. Check Web App URL / permissions.\n" + err.message);
  } finally {
